...
// Save and Restore ios fill and precision, and set width to zero, as if used.
#include <boost/io/ios_state.hpp> // Extra manipulators.

#include <boost/quan/unc.hpp> // Uncertain classes.
#include <boost/quan/impl/unc_io.hpp>
...
namespace boost {
  ...
  /*!
    ...
    \exception Throws `std::bad_alloc()` if 'os::iword' has not been initialised correctly by setUncDefaults function,
  ...
  */
      ...
      std::ostream& os = std::cout)  // Output stream.
    {
      ...
      { // iwords are corrupt!
        ...
        // declared in unc_fwd.hpp and defined in unc_io.hpp
        os << "??? !" << endl;
  ...
  // Check value features first.
      ...
      if (!boost::math::isfinite(static_cast<double>(value)))
      ...
      { // value IS finite, so can test if value is negative or zero, too big or too small.
        ...
        { // is Negative (OK to use < 0. test here because have already eliminated NaN and inf).
          ...
          using std::abs;
          value = abs(value); // Can now ignore effect of negativeness, adding sign for output.
        }  // value < 0.

        isValueNearZero = (value < std::numeric_limits<double>::min() * 10.);
      ...
      { // stdDev is NaN or infinity.
        ...
        { // If stdDev == infinity, then, by convention, value == NaN.
          value = std::numeric_limits<double>::quiet_NaN(); // So the value becomes undefined too.
          isValueNaN = true;
          isStdDevInf = true;
        } // stdDev is NaN or infinity.
      } // stdDev NOT finite.
  ...
  // 0 == scale10 means (not)scaled by unity. 3 means divided by 1000 ...
      ...
      { // Show full precision, but NO trailing zeros, so get "2.54" not "2.5400000000000"
        sigDigits = 1;
        // sigDigits = std::numeric_limits<double>::digits10; // Typically 15 digits are guaranteed.
