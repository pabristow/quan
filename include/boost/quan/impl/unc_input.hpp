/*!
  ...
  \details Class for simple Propagation of Uncertainties
     ...
     \details stream input function of uncertain types.
*/
...
#ifndef BOOST_UNC_INPUT_HPP
#define BOOST_UNC_INPUT_HPP
...
*/
      ...
      std::istream& is = std::cin) // Default input stream.
    {
      ...
      { // No integer part of value.
        ...
        value = std::numeric_limits<double>::quiet_NaN();
        stdDev = std::numeric_limits<float>::quiet_NaN();
        using boost::quan::unc_types;
        types = static_cast<unsigned short>(~(unc_types::VALUE_ZERO | VALUE_INTEGER | VALUE_RATIONAL | VALUE_NEGATIVE_ONLY | VALUE_POSITIVE_ONLY | UNC_KNOWN | UNC_NOPLUS
          | UNC_NOMINUS | UNC_QUAN_DECIMAL | UNC_QUAN_BINARY | UNC_EXPLICIT | UNC_UNIFORM | UNC_TRIANGULAR | DEG_FREE_EXACT | DEG_FREE_KNOWN));
      ...
      types |= boost::quan::unc_types::UNC_KNOWN;
    ...
    //   types |= (UNC_KNOWN | VALUE_INTEGER | VALUE_RATIONAL | UNC_NOPLUS | UNC_NOMINUS);
      ...
      { // Uncertainty (standard deviation) and degrees of freedom may follow.
        ...
        { // Some uncertainty, for example: +0.123, -0.456 or "+/-2.34"
          std::char_traits<char>::int_type c_sign = is.get(); // '+' or '-' read and discard.
          if (c_sign == '+')
          {
            isPlus = true;
          }
          ...
          { // Skip over any separator.
            std::char_traits<char>::int_type c_sep = is.get(); // '/' or '|' or '\' chars.
            if (c_sep == 0x0F1)
