/*! \file
   \brief Class for simple Propagation of Uncertainties according to a pure Gaussian model.
    ...
    A fuller collection of even fancier classes also given in his unc.h.
 ...
 //!   double confidence = os.iword(conf) / 1000.;  // `<< confidence(0.05)` aka 95%
    ...
    { // Want to append confidence interval as <1.23, 2.34>.
      ...
      { // Possible to compute confidence limits or interval in < > angle brackets.
        ...
        double epsilon_ci = os.iword(roundingLossIndex) / 1.e3; // Pick up and rounding loss and unscale.
...
/*
Version 1.29.0: Boost.Function has been partially redesigned to minimize the interface and make it cleaner.
Several seldom- or never-used features of the older Boost.Function have been deprecated and will be removed in the near future.
Here is a list of features that have been deprecated, the likely impact of the deprecations, and how to adjust your code:

The boost::function class template syntax has changed.
The old syntax, e.g.,

   boost::function<int, float, double, std::string>,

has been changed to a more natural syntax

   boost::function<int (float, double, std::string)>,

where all return and argument types are encoded in a single function type parameter.

Any other template parameters (e.g., the Allocator) follow this single parameter.

The resolution to this change depends on the abilities of your compiler:
if your compiler supports template partial specialization and can parse function types (most do),
   modify your code to use the newer syntax (preferable)

or directly use one of the functionN classes whose syntax has not changed.
If your compiler does not support template partial specialization or function types,
you must take the latter option and use the numbered Boost.Function classes.
This option merely requires changing types such as

boost::function<void, int, int> to boost::function2<void, int, int>
(adding the number of function arguments to the end of the class name).
*/

template<typename Type> // Predicate Functor modelled on STL less in functional.
// Used below by min_element, etc.
// struct lessAbs : public boost::functional::detail::binary_function<Type, Type, bool>
// replaced with std::binary_function - but is deprecated in C++11 and will be removed.
// Unclear how to fix properly.
