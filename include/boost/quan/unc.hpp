...
#ifndef BOOST_UNC_HPP
#define BOOST_UNC_HPP
...
#include <algorithm>
  //using std::max;
  //using std::min;
#include <string> // char_traits<char>
#include <sstream> //
...
//! operator<< defined in unc_io.hpp
//! Usage: \code out << showUncTypes(unc) ...class showUncTypes \endcode
class showUncTypes
{ 
  friend std::ostream&operator<< (std::ostream&, const showUncTypes&);
public:
  showUncTypes(unsigned short int types) : types_(types)
  {
    // Constructor only initializes types_.
  };
...
public:
  setAllUncFlags(int flags) : flags_(flags)
...
public:
  setMaskedUncFlags(int w, int m) : flags(w), mask(m) {} // Constructor initialisation flags = w.
...
public:
  resetMaskedUncFlags(int w, int m) : flags(w), mask(m) {} // Constructor initialisation flags.
  int flags; // setMaskedUncFlags.flags used by operators << and >>
  int mask;  // Selected bits to deal with by zeroing before setting flags.
};
...
public:
  setSigDigits(int sigDigits) : sigDigits_(sigDigits) {} // Constructor sigDigits = w;
...
*/
//! \tparam is_correlated if true, standard deviation is correlated, else not (default = false).
...
/*! Allow value part of variables of class unc to be assigned to, 
    and compared with double.
\tparam T Built-in floating-point type, float, double or long double, 
     or uncertain type unc or Meas.
*/
//template <class T>
//double value_of(T);
...
//{ //! \return value as a @c double (it was already, so does nothing for type @c double).
//  return v;
//}
...
// Pair functions.
//! \returns as a pair of @c doubles, the value element of argument provided.
//! If a built-in type, then is just a conversion to @c double.
//! If an uncertain type, unc or Meas, then just the value element, 
//!  ignoring estimate of uncertainty, degrees of freedom etc.
...
{ //! \return values (parts) as a pair of doubles.
  ...
  std::pair<double, double> minmax = 
     std::make_pair(up.first.value(), up.second.value());
  return minmax;
}
...
//// Is this the BOOST_MATH_STD_USING fix?
