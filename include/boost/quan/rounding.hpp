...
// Some constants (might be added to Boost::math::constants?)
...
{ /*! \brief Round floating-point v (not-exponential) to order m 
  ...
  */
  ...
  MSVC had always 3 exponent digits but now perhaps it follows gcc in using 2 or 3
  ...
  { // for double == 22 (would be 23 if value was negative, but we are only dealing with abs value).
    ...
    std::cout << "\n" << v << ", " << m << ", " << s << ", "
      ...
      + 1 + 1 + 5 // 1 sign, 1 decimal point, e+00
      << std::endl;
    // assert(s.size() == std::numeric_limits<FPT>::digits10 + 1 + 1 + 5);
    // Triggers assert 15Dec2016 for reasons unclear.
...
double gamma(double rounded, double value) 
{
  ...
  \sa Measurement Science and Technolology, 11 (2000) 1659-1665. ISSN 0957-0233 S0957-233(00)13838-X.
    Gejza Wimmer, Viktor Witkovsky, Tomas Duby,
    Proper rounding of the measurement results under normality assumptions.

  \param rounded Value (as @c double) after rounding.
  \param value Value (as @c double) before rounding. Assumes @c rounded < @c unrounded value`.
