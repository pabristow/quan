...
{ 
  std::string message("Quan1 test: " __FILE__ );
#ifdef __TIMESTAMP__
  ...
  BOOST_CHECK(std::cout.iword(zeroIndex) == indexID); // Should pass.
  BOOST_CHECK(std::cout.iword(topIndex) == indexID); // Should pass.
  std::cout.iword(0) = indexID; // 1st iword[0] to hold a 'magic' id.
...
BOOST_AUTO_TEST_CASE(quan1_test_1)
  ...
  }
  {
    ...
    //std::cout << "uncun u2(2., 0.F) == " << u2 << std::endl;
  }
  {
    std::ostringstream oss;
    setUncDefaults(oss);
    ...
    // std::cout << "uncun u123(1.23., 0.01F) == " << plusminus << u123 << std::endl;
  }
} // BOOST_AUTO_TEST_CASE(quan1_test_1)
...
{ // Tests on display of uncertain flags and uncertain types. 
    ...
    //std::cout << showUncFlags(uncFlags) << std::endl;
    CHECK(showUncFlags(uncFlags), "uncFlags (48) add_+/- add_noisy."); 
    unsigned short int uncTypes = UNC_QUAN_DECIMAL | UNC_KNOWN;
    //std::cout << showUncTypes(uncTypes) << std::endl; // uncTypes: (120) uncKnown quantize10
    ...
    //std::cout << showUncTypes(defType) << std::endl; 
...
BOOST_AUTO_TEST_CASE(quan1_test_3)
{
  ...
  {// Test of comparison operators and predicates for uncs now in unc.h. 
   ...
   uncun u0(0.,0.F); // Exact integral zero gave problems before, now OK - all zero.
    std::cout << noplusminus << std::endl; 
    std::cout << "uncun u0(0.,0.F) == " << u0 << std::endl;  // plain 0
    std::cout << plusminus << flush;
    std::cout << "uncun u0(0.,0.F) == " << u0 << std::endl; // still plain 0 as expected.
    std::cout << noplusminus << u0 << std::endl; // still plain 0 as expected.
    std::cout << plusminus  << u0 << std::endl; // 9.89 < 9.99 < 10.1
    std::cout << addlimits << plusminus << u0 << std::endl; // 9.89 < 9.99 +/-0.10 < 10.1
    std::cout << uppercase << boolalpha << std::endl;
    std::cout << showformat << std::endl; // FormatFlags (0x4205)  skipws, uppercase, dec, boolalpha.
    // 	ostream& showuncflags(ostream& os);
    std::cout << showuncflags << std::endl; // uncFlags (0x808)  add +/-, add limits.
  ...
  if(false)// Test to_string, and some specializations.
  {
    ...
    std::cout << xx << std::endl; // to_string default is precision(10) not std default 6.
    CHECK(xx, "3.333333333"); // Expect precision 10 digits.
    xx = to_string<bool>(true); 
    std::cout << xx << std::endl; // to_string default is boolalpha.
   ...
   uncun uu(1.6, 0.1f);
    ...
    //std::string hs = "to_string<uncun>(hhh)  " + to_string<uncun>(hhh) +" is cool!";
 ...
 uncun hhh(1.6, 0.1f); // even at 2 standard deviations.
  ...
  std::cout 	<< "top of lower " << l.value() << " = " << l.value() + l.std_dev() << std::endl
    << "bottom of higher " << h.value() << " = " << h.value() - h.std_dev() << std::endl;
  ...
  std::cout << "Difference of values in max sds between " << plusminus << h << " and " << l << " == "
   << (h.value() - l.value())/ std::max(l.std_dev(), h.std_dev()) << std::endl; // max

  std::cout << "Difference in sds between " << h << " and " << l << " == "
   << (h.value() - l.value())/ hypot(h.std_dev(), l.std_dev())  << std::endl; // hypot

  std::cout << "Difference in sds between " << hh << " and " << ll << " == "
   << (hh.value() - ll.value())/ hypot(hh.std_dev(), ll.std_dev())  << std::endl; // hypot

  std::cout << "Difference in sds between " << hhh << " and " << lll << " == "
   << (hhh.value() - lll.value())/ hypot(hhh.std_dev(), lll.std_dev())  << std::endl; // hypot
 ...
 uncun meandiffu;
  meandiffu = ( hh - ll);
  std::cout << "Means "  << plusminus << ll << tab << hh << ", Difference of means is " << meandiffu << std::endl;
  std::cout << "SDs " << ll.std_dev() << tab << hh.std_dev() << std::endl;
...
//	std::cout << "Probability that are different is " << prob << std::endl;


  if (l < h)
  { // Usesuncun::operator<  - only uses central value, no uncertainty.
    std::cout << noplusminus << l << " is  < " << h  << std::endl; // 0.90 < 1.1 < 1.3  is < 1.0 < 1.2 < 1.4
  }
  if (l == h)
  { // Usesuncun::operator==  - only uses central value, no uncertainty.
  ...
  {
    std::cout << "(less<double>(" << ld << ", " << hd << " )) " << std::endl;
  }
  if (std::less<uncun>()(l, h))
  {
    std::cout << "(std::less(l, h)) " << " l < h " << std::endl;
  }
  if (lessAbs<uncun>()(l, h))
  {
    std::cout << "(lessAbs(l, h)) " << " l < h " << std::endl;
  }
  std::cout << "Test member predicate function " << plusminus << l << " < " << h  << std::endl;
  ...
  std::cout << " l " << l << ((uncun::equalU(l, h) ? " ~= " : " != ")) << " h " << h << std::endl;
  std::cout << " ll " << ll << ((uncun::equalU(ll, hh) ? " ~= " : " != ")) << " hh " << hh << std::endl;
  std::cout << " l " << l << ((uncun::equalU2(l, h) ? " ~= " : " != ")) << " h " << h << std::endl;
  std::cout << " ll " << ll << ((uncun::equalU2(ll, hh) ? " ~= " : " != ")) << " hh " << hh << std::endl;
...
//	typedef unc<false>::lessU UlessU;  doesn't work
  ...
  {    // Class Meas tests.
    ...
    //std::cout << "Meas zz; = " << zz << std::endl;
    CHECK(zz, "0"); 

    zz = 1.;
    //std::cout << "zz = 1; == " << zz << std::endl;
    CHECK(zz, "1."); 

    Meas z0(0.); // Construct from double zero.
    //std::cout << "Meas z0(0.) == " << z0 << std::endl;
    CHECK(z0, "0"); 

    Meas z0i(0); // Construct from int zero.
    //std::cout << "Meas z0i(0) == " << z0i << std::endl;
    CHECK(z0i, "0"); 
    ...
    //std::cout << "Meas m0(u0, ""m0"", now0); = " << m0 << std::endl;
    ...
    //std::cout << zz <<  " < " << z0 << " is " << boolalpha << c << std::endl;
    //c = (zm1 < 0 );
    // BOOST_CHECK(zm1 < 0 );
    // BOOST_CHECK(0 > zm1); // No operator > defined as yet.

    //std::cout << zm1 <<  " < " << z0 << " is " << boolalpha << c << std::endl;
    //bool c = (zm1 < Meas(0) );
    BOOST_CHECK(zm1 < Meas(0) );
    // std::cout << zm1 <<  " < " << Meas(0) << " is " << boolalpha << c << std::endl;
    BOOST_CHECK(Meas(0) > zm1 );

    Meas one(1);
    Meas mone = -one;
    //std::cout << "-one == " << mone << std::endl;
    CHECK(mone, "1.");
    //std::cout << "Meas zm1(-1.) == " << zm1 << std::endl;
    CHECK(zm1, "-1.");
    ...
    //std::cout << "abs(zm1) = std::abs(zm1) = " << abs(zm1) << std::endl; // Should lookup type Meas, and does!
    //std::cout << "abs<Meas>(zm1) == " << abs<Meas>(zm1) << std::endl; // explicit Type Meas fails!
    //std::cout << "abs(zm1).m_value == " << abs(zm1.m_value) << std::endl;		

  //Meas lmNow(l, "low", now);
  //std::cout << "Meas lmNow(l, \"low\", now) == " << lmNow << std::endl; 
  //Meas hmNow(h, "hi", 2);
  //std::cout << "Meas hmNow(h, \"hi\", 2); == " << hmNow << std::endl; 
  //time_t nnow =   now + 90; // Add 90 seconds to time for Meas hi.
  //Meas nmNow(h, "hi", nnow); // Show time has advanced.
  //std::cout << "Meas nmNow(h, \"hi\", nnow); == " << nmNow << std::endl; // Meas nmNow(h, "hi", nnow); == hi 1.2 @ 22:59:49
  } //  BOOST_AUTO_TEST_CASE(quan1_test_4)
  ...
  {  // Some comparisons.
   ...
   uncun h(1.15, 0.06f);
    ...
    BOOST_CHECK(lm < hm);  // 
    std::cout << "(lm < hm)  "<< lm << " < " << hm << std::endl; // (lm < hm)  low 1.1 #1 < high 1.15 #2

    BOOST_CHECK(std::less<Meas>()(lm, hm));  // 
    // Using std functor less which uses operator<
    std::cout << "std::less<Meas>()(lm, hm) " << lm << " < " << hm << std::endl;
    ...
    std::cout << "lessAbs<Meas>()(lm, hm) " << lm << " < " << hm << std::endl;
    ...
    // Compare absolute value within uncertainty using Meas::lessAbsM function.
      std::cout << "Meas::lessAbsM(lm, hm) " << lm << " < " << hm << std::endl;
  ...
  if (!fin.is_open())
  {
    std::cerr << "File " << testInputFilename << " failed to open!" << std::endl;
    return;
  }
  std::cout << "Quan1 Test input from " <<  testInputFilename << '  '<< __TIMESTAMP__  << std::endl;
 
  // +/- symbol = dec 177, hex F1  messy!! fout is different to std::cout!
  std::ofstream fout(outFilename, ios::out); // use default overwrite/ iso::replace.
  BOOST_CHECK(fout.is_open());
  if (!fout.is_open())
  {  // File failed to open OK.
    std::cerr << "File " << outFilename << " failed to open!" << std::endl;
    return;
  }
  std::cout << "Quan1 Test output to "	<< outFilename << '  '<<  __TIMESTAMP__ << std::endl;

  int v_count = 0;  // 
  vector<uncun> vs;
  vs.reserve(20); // More efficient than repeated push_backs.
  std::cout << "'  'reserved for " << vs.capacity() << " values." << std::endl;
  ...
  while(!fin.fail())
  {
    ...
    { // Not a blank line.
      ++lineCount;
      fout << "Input line: " << line << std::endl;  // Echo input line to file.
      std::istringstream is(line);
      do
      {
        uncun v;
        is >> v; // Read uncertain value "1.2" or "1.2 +/-0.05".
        //std::cout << v << std::endl;  // Echo inputuncun with +/-.
        ...
        {  // Tried to read more but no units given.
          std::cout << "is.eof() == true" << " after " << v << " (no unit)." << std::endl;
          break;
        } // if
        if (is.fail() == true)
        { // Always seems to be fail == true when eof.
          std::cout << "is.fail == true!" << std::endl;
          std::cout << v << "(no unit)." << std::endl;
          break;
        }
   ...
   //std::cout << "vs.front() == " << vs.front() << ", vs.back() == " << vs.back() << std::endl;
   //std::cout << "*vs.begin() == " << *vs.begin() << ", *vs.end() -1 == " << *(vs.end() -1) << std::endl;
  ...
  fout << nl << v_count << " uncertain values with expected uncertainty "
    ...
    << expectUnc
    << ", input from file: "  << testInputFilename << std::endl;

  std::copy(vs.begin() + 1, vs.end(), std::ostream_iterator<uncun>(fout, "\n"));
  // vs.begin() is vs[0] which is expected uncertainty.
  fout << std::endl;  // vs[1] = 1.10 1.20 1.00 ... 

  fout << vms.size()-1 << "  measurements." << std::endl;
  ...
  { // Sum uncertain values using while loop.
    std::cout << v_count << " Uncertain Values:" << plusminus << addlimits << std::endl;
    ...
    std::cout << "mean = " << sum.value() / i << std::endl; 
  }
    
    std::ostream_iterator<Meas> osmit(std::cout, ", ");  // Set up iterator and comma separator.
    std::cout.iword(uncFlagsIndex) &= ~plusMinus;  // clear bit 3 = 0 to mean Do NOT add +/-.
    std::cout << showuncflags << std::endl;
    std::cout << nl << "plusMinus " << noplusminus << nolimits << showuncflags << std::endl;
  ...
  // Examples of std::sorting by various criteria.
    ...
    std::copy(vms.begin() +1, vms.end(), osmit);  std::cout << std::endl;
    ...
    std::cout << '\n' << "plusMinus " << plusminus << showuncflags << vs.at(0) << std::endl;
...
//		bool Meas::lessU2(const Meas& l, const Meas& r); // Needs to be static member function.
    ...
    std::cout << "std::sorted by value using static member functor Meas::less." << nl;
    std::copy(vms.begin() +1, vms.end(), osmit);	std::cout << nl << std::endl;
    ...
    std::cout << '\n' << __FILE__ << " ended " <<  __TIMESTAMP__ << std::endl;
  } // BOOST_AUTO_TEST_CASE(quan1_test_6)

  BOOST_AUTO_TEST_CASE(quan1_test_7)
  { // Obselete test of C arrays.
  ...
  //  std::cout << v_count << " Uncertain Values input from file:" << plusminus << adddegfree << std::endl;
  ...
  //  while(vi != vs.end())
  //  {
  //    // std::std::cout << "Value " << i << '  '<< *vi << std::endl; // Value 1 1.10, ...
  //    ++i;
  //    ++vi;
  //  }  // while, leaving vi == vs.end();
  ...
  //    std::cout << (((*vi).types() & UNC_EXPLICIT) ? "explicit" : "implicit" ) << std::endl;
  //    vvs[count] = (*vi).value();
  //    vus[count] = (*vi).std_dev();
  //  }	// for
  //  std::cout << std::endl; // Value 2 1.20  Value 3 1.00
  //}
  //
  //std::cout << setprecision(16) << std::endl;  // Full precision, including extra 'noisy' digit.
  //
  ...
  //        << " uncertainty "
  //        << vus[count]
  //        << " is greater than expected "<< vs[0] << '!' << std::endl;
  //    }
  //  }	// for
 ...
 {  // Change in global locale, and thus that imbued to input and output,
  ...
  //std::cout << krautLocale.name( ) << " truename "<< npunctK.truename() << std::endl;
  //std::cout << aussieLocale.name( ) << " falsename "<< npunctK.falsename() << std::endl;
  ...
  //std::cout << " v(1.234, 0.002, 20) == "<< v << std::endl; // v(1.234, 0.002, 20) == 1,2340 Note , !
 ...
 { // other code to handle file input?
  ...
  std::cout << "'  'for " << vs.size() << " values." << std::endl;
  int vNumber = 0;
  ...
  {  // do all tests.
    ...
    std::istringstream myIstream(line); // "  1.2 +/- 0.1 ";
