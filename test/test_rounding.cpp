...
BOOST_AUTO_TEST_CASE(cdf_quantile_test)
  { // Test the calculation of CDF and quantiles for triangular and uniform distributions.
    static const double sqrt6 = 2.4494897427831780981972840747058913919659474804966; // sqrt(6)

    boost::math::triangular tdist(-sqrt6, 0., +sqrt6); // Wimmer page 23, below eq 6.
    double alpha = 0.005;
...
{  // Wimmer Triangular distribution example 3, page 27.
  ...
  // This is different on MSVC using debug, but OK using release.
  ...
  // Suspect this may be caused by differences between stdio for GCC and Clang -using <fmt> is probably better, but needs C++20.
